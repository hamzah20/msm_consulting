...
class Pph21 extends CI_Controller
{
	...
	{
		// Cek tanggal resign
		$cek_resign = $this->cms->getSingularData('v_g_employee', 'EMPLOYEE_ID', $this->input->get('eid'));
		foreach ($cek_resign->result() as $resign);
		$tanggal_resign = $resign->EMPLOYEE_WORK_END;
		if($tanggal_resign == NULL){
			$tanggal_resign = '0000-00-00';
		} 
		
		// Expload tanggal resign
		$exp_tanggal_resign = explode("-", $tanggal_resign);
		$exp_resign_years   = $exp_tanggal_resign[0];
		$exp_resign_month   = $exp_tanggal_resign[1]; 
		
		// Convert bulan resign 
		$monthName = $this->incube->convertMonthNumber($exp_resign_month);

		// Cek periode (bulan dan tahun)
		$cek_periode = $this->cms->getSingularData('g_pph21', 'PPH_ID', $this->input->get('pid'));
		foreach ($cek_periode->result() as $periode);
		$periode_years  = $periode->PERIOD_YEAR;
		$periode_month = $periode->PERIOD_MONTH; 

		// Cek jika periode pph == periode resign maka status masa pajak terakhir
		if($exp_resign_years == $periode_years && $monthName == $periode_month){
			$status = "MASA PAJAK TERAKHIR";
		}else{
			$status = "MASA";
		} 
		// atau jika periode bulan == desember, maka status masa pajak terakhir
		if($periode_month == 'Desember'){
			$status = "MASA PAJAK TERAKHIR";
		}

		$data['status_resign'] = $status;

		$data['employee']	= $this->cms->getSingularDataDetail('v_g_employee_pph21', 'EMPLOYEE_ID', 'PPH_ID', trim($this->input->get('eid')), trim($this->input->get('pid'))); 

		$data['employee_year']	= $this->cms->getSingularData('v_g_employee_pph21_yearly', 'EMPLOYEE_ID', trim($this->input->get('eid')));
	...
	{ 
		...
		// $data['summary'] 	= $this->cms->getGeneralData('v_g_companies_pph21_detail_ptt', 'PPH_ID_PTT', $this->input->get('pid_ptt'));
		$data['summary'] 	= $this->cms->getGeneralData('v_g_companies_pph21_detail', 'PPH_ID', $this->input->get('pid'));
		$data['summary_ptt'] 	= $this->cms->getGeneralData('v_g_companies_pph21_detail_ptt', 'PPH_ID_PTT', $this->input->get('pid_ptt'));
	...
	public function importXLSLFilePTT()
	{
		...
		$pphID = $this->input->post('pid_ptt');
		$pid_ptt = $this->input->post('pid_ptt');

		$companyCheck  = $this->cms->getSingularData('v_g_companies', 'COMPANY_ID', $this->input->post('companyID'));
		// $employeeCheck = $this->cms->getSingularData('g_pph21_detail_ptt', 'COMPANY_ID', $this->input->post('companyID'));
		$pphCheck 	   = $this->cms->getSingularData('g_pph21_detail_ptt', 'PPH_ID_PTT', $pphID); 
		...
		foreach ($sheet as $sheetData) {
			//Kalo ada
			$cek_employee_id  = $this->cms->getSingularData('g_pph21_detail_ptt', 'EMPLOYEE_ID_PTT', $sheetData['B']);
		...
		redirect('pph_21/bulan/summary/tidak_tetap?pid=' . $this->input->post('pid') . '&cid=' . $this->input->post('companyID'). '&mid=' . $this->input->post('monthID'). '&yid=' . $this->input->post('yearID') . '&pid_ptt=' . $pphID);
	...
	public function generateXLSFilePTT()
	{
		...
		$employeeData = $this->cms->getSingularDataDetail('g_pph21_detail_ptt', 'COMPANY_ID', 'PPH_ID_PTT', $companyID, $pid_ptt);
	...
	{
		// Ambil ID Company dan Periode Tahun
		$cid=$this->input->get('cid');
		$yid=$this->input->get('yid');

		// Ambil semua data karyawan, company, dan hasil tahunan di company dan periode tahun
		...
		foreach($getDataKaryawan->result() as $data1) {
			...
			$bijab_year = ($totalBruto_year*5)/100;
			...
			$companyData = array(
				...
				'EMPLOYEE_BIAYA_JABATAN_YEAR'		=> $bijab_year, 
	...
	public function importXLSLFile() 
	{
		...
		foreach ($employeeCheck->result() as $employee) {
			...
			$tmpArr = array(
				...
				'INTERNAL_ID'					=> $employee->EMPLOYEE_INTERNAL_ID,
				'ID'							=> $employee->EMPLOYEE_ID, 
				'EMPLOYEE_WORK_START'			=> $employee->EMPLOYEE_WORK_START, 
				'EMPLOYEE_WORK_END'				=> $employee->EMPLOYEE_WORK_END, 
				'EMPLOYEE_NATIONALITY_STATUS' 	=> $employee->EMPLOYEE_NATIONALITY_STATUS,
			);
		...
		foreach ($sheet as $sheetData) {
			...
			if (($dataPeriodYears == $cekPPH21Years) && ($monthName == $cekPPH21Month)) {
				...
				// Jika tahun dan bulan resign bukan pada periode sekarang
				$totalBruto 	= $sheetData['E'] + $totalTunjangan + $sheetData['Q'] + $totalPremi + $sheetData['U'] + $sheetData['V'];
				$totalBrutoNoTHR 	= $sheetData['E'] + $totalTunjangan + $sheetData['Q'] + $totalPremi + $sheetData['U'];

				// Disetahunkan sesuai periode bulan masuk
				// Jika masuk nya bulan januari atau kosong berarti 12 bulan
				// Jika masuk nya bulan februari berarti 11 bulan (11) 
				
				// Mengambil periode masuk karyawan jika ada 
				$cekWorkStart = $employeeArr[$employeeIndex]['EMPLOYEE_WORK_START']; 

				if ($cekWorkStart) {
					$dataPeriod = explode('-', $cekWorkStart);
					$dataPeriodYears = $dataPeriod[0];
					$dataPeriodMonth = $dataPeriod[1]; 
				} else{
					$dataPeriodYears = '0000';
				...
				// Mengconvert number month menjadi month name
				$monthName   = $this->incube->convertMonthNumber($dataPeriodMonth);
				$monthNumber = $this->incube->convertMonthNameLP($monthName);

				if($employeeArr[$employeeIndex]['EMPLOYEE_NATIONALITY_STATUS'] == 'Ekspatriat'){
					$totalBulan = 12;
				} elseif ($employeeArr[$employeeIndex]['EMPLOYEE_NATIONALITY_STATUS'] == 'Lokal') {
					if ($dataPeriodYears < $cekPPH21Years) {
						$totalBulan = 12;
					} elseif ($dataPeriodYears == $cekPPH21Years) { 
						$totalBulan = 13 - $monthNumber;
					} else{
						$totalBulan = 12;
					}  
				}
				...
				//masukan dari saya untuk koding bijab nonthr dan totalpengurang nonthr
				$bijabNoTHR = 0.05 * $totalBrutoNoTHR;
				if($bijabNoTHR>500000) {
					$bijabNoTHR= 500000;
				}
				$totalPengurangNoTHR	= $bijabNoTHR + $iuran;
				...
				echo "bruto".$totalBruto;
				$yearlyNeto = ($totalBulan * ($totalBruto - $totalPengurang - $sheetData['V']))+$sheetData['V'];

				//masukan dari saya untuk koding bijab nonthr dan totalpengurang nonthr untuk yearlyNetoNoTHR
						//$yearlyNetoNoTHR = ($totalBulan * ($totalBrutoNoTHR - $totalPengurangNoTHR ));

				$yearlyNetoNoTHR = ($totalBulan * ($totalBrutoNoTHR - $totalPengurang ));
				$yearlyPKP  = $yearlyNeto - $ptkpTarif;
				$yearlyPKPNoTHR  = $yearlyNetoNoTHR - $ptkpTarif;

				//Dibulatkan biar 3 digit dibelakang jadi 0
				$yearlyPKP = (floor($yearlyPKP / 1000)) * 1000;
				$yearlyPKPNoTHR = (floor($yearlyPKPNoTHR / 1000)) * 1000;
				echo "<br/>PKP".$yearlyPKP;
				...
				if ($yearlyPKPNoTHR >= 0) {
				    if ($yearlyPKPNoTHR > 500000000) {
				        $tier1 = 0.05 * 50000000;
				        $tier2 = 0.15 * 200000000;
				        $tier3 = 0.25 * 250000000;
				        $tier4 = 0.3 * ($yearlyPKPNoTHR - 500000000);
				        $yearlyPPHnoTHR = $tier1 + $tier2 + $tier3 + $tier4;
				    } elseif ($yearlyPKPNoTHR > 250000000) {
				        $tier1 = 0.05 * 50000000;
				        $tier2 = 0.15 * 200000000;
				        $tier3 = 0.25 * ($yearlyPKPNoTHR - 250000000);
				        $yearlyPPHnoTHR = $tier1 + $tier2 + $tier3;
				    } elseif ($yearlyPKPNoTHR > 50000000) {
				        $tier1 = 0.05 * 50000000;
				        $tier2 = 0.15 * ($yearlyPKPNoTHR - 50000000);
				        $yearlyPPHnoTHR = $tier1 + $tier2;
				    } else {
				        $tier1 = 0.05 * $yearlyPKPNoTHR;
				        $yearlyPPHnoTHR = $tier1;
				    }
				}		
				if (strlen($employeeArr[$employeeIndex]['NPWP']) == 0) {
					//Kalau misalnya pegawai ga punya NPWP
					$yearlyPPH = $yearlyPPH  * 1.2;
					$yearlyPPHnoTHR = $yearlyPPHnoTHR  * 1.2;
				} 
				$PPHTHR=$yearlyPPH-$yearlyPPHnoTHR;
				
				echo "pph year:".$yearlyPPH."<br/>";
				echo "pph year no thr".$yearlyPPHnoTHR."<br/>";
				echo "pph thr".$PPHTHR."<br/>";
				

				$monthlyPPH = ($yearlyPPHnoTHR / $totalBulan) + $PPHTHR; 
				$monthlyPPHFinal=$monthlyPPH;
				echo $monthlyPPHFinal."<br/>";
				...
				if ($companyCheck->row()->PPHCOUNT_METHOD == 'GROSS UP') {
					...
					while ($Tunjpph != $monthlyPPH) { 
						...
						$Tunjpph=$monthlyPPH;
						$totalBruto 	= $sheetData['E'] + $Tunjpph + $totalTunjangan + $sheetData['Q'] + $totalPremi + $sheetData['U'] + $sheetData['V'];  
						$totalBrutoNoTHR 	= $sheetData['E'] + $Tunjpph + $totalTunjangan + $sheetData['Q'] + $totalPremi + $sheetData['U'] ;  
						...
						$yearlyPKP  = $yearlyNeto - $ptkpTarif;
						$yearlyPKPNoTHR  = $yearlyNetoNoTHR - $ptkpTarif;
						echo "<br/>netto2:".$yearlyNeto;
						//Dibulatkan biar 3 digit dibelakang jadi 0
						$yearlyPKP = (floor($yearlyPKP / 1000)) * 1000;
						$yearlyPKPNoTHR = (floor($yearlyPKPNoTHR / 1000)) * 1000;
						echo "<br/>pkp2:".$yearlyPKP."<br/>";
						...
						echo "totalbruto :". $totalBruto;
						echo "totalbrutonothr". $totalBrutoNoTHR;
						...
						//tambahan dari saya untuk membetulkan Gross Up dengan THR
						$PPHTHR=$yearlyPPH-$yearlyPPHnoTHR;

						$monthlyPPH = ($yearlyPPHnoTHR / $totalBulan)+$PPHTHR;
						$monthlyPPHFinal=$monthlyPPH;
						echo "<br/>pph2.1:".$monthlyPPH;
			...
			$employeeData = array(
				...
				//tambahan dari saya untuk membetulkan view summary bulanan kalkulator masa nomor 17 Penghasilan Neto Setahun/Disetahunkan dengan kode dibawah ini

				//'EMPLOYEE_NETTO_YEAR'			=> (floor($totalBruto) - $totalPengurang - $sheetData['V'])*$totalBulan+$sheetData['V'],
		...
		$this->pph_21_tahun_onlycheck($this->input->post('companyID'),$this->input->post('yearID'));

		redirect('pph_21/bulan/summary?pid=' . $pphID . '&cid=' . $this->input->post('companyID'). '&mid=' . $this->input->post('monthID'). '&yid=' . $this->input->post('yearID'));
	}

	public function pph_21_tahun_onlycheck($cid,$yid)
	{
		// Ambil ID Company dan Periode Tahun
		// $cid=$this->input->get('cid');
		// $yid=$this->input->get('yid');
		...
		foreach($getDataKaryawan->result() as $data1) {
			...
			$totalBruto_year 	= $gaji_pokok + $totalTunjangan_year + $honarium + $totalPremi_year + $natura + $tantiembonus;
			$bijab_year = ($totalBruto_year*5)/100;
			$bijab_year_max = 500000*$ttl_month;
			if($bijab_year > $bijab_year_max){
				$bijab_year = $bijab_year_max;
			} else{
				$bijab_year = $bijab_year;
			}
